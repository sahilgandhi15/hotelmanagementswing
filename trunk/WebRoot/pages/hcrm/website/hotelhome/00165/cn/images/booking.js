...
var hotelCode="00165", language="chs";//code根据酒店进行修改
...
function validate(){
	...
	f.loginName.value=f.loginName.value;
	f.password.value="";
	f.loginName.value="";
	return true;
}
...
function guest_login(){
	...
	GB_showCenter("邯郸水星酒店",url,550,650);//酒店名字，550是高，650是宽，根据需要修改。
}
//判断登录结束
...
function queryPrice(){
	...
	GB_showCenter("邯郸水星酒店",paction,550,650);//酒店名字，550是高，650是宽，根据需要修改。
	//f.action=paction;
	//this.href=paction;
	//f.target="_blank";
	//f.submit();
...
function findPassword(){
 ...
 GB_showCenter("邯郸水星酒店",url,450,650);//酒店名字，550是高，650是宽，根据需要修改。
}

//注册
function register(){
	var url = "http://localhost/HotelManagement_web/apps/register.action";
	GB_showCenter("邯郸水星酒店",url,550,650);//酒店名字，550是高，650是宽，根据需要修改。
}
...
function GB_showCenter(title, url, hight, width) {
	return window.showModalDialog(url, "", "dialogHeight=" + hight + "px; dialogWidth=" + width + "px;status=no");   
}
