...
$import("/hcrm/website/hotelhome/js/calendar/en/calendar.js","js");//日历
$import("/hcrm/website/hotelhome/js/md5.js","js");//登录加密

var hotelCode="00165", language="enu";//根据实际的Code改动
...
function guest_login(){
	...
	GB_showCenter("Enjoyable Stars Hotel",url,550,650);//酒店名字，550是高，650是宽，根据需要修改。
}
//判断登录结束
...
function queryPrice(){
	...
	if(f.checkInDate.value<gGetToday()){
		alert("checkInDate date can't be earlier than now !");
	...
	if(f.checkInDate.value>=f.checkOutDate.value){
		alert("checkOutDate date can't be earlier than CheckInDate date !");
		return false;
	}
	if(f.checkOutDate.value>gDateAdd(f.checkInDate.value,1,0,0)){
		alert("checkInDate must more than one year !");
	...
	GB_showCenter("Enjoyable Stars Hotel",paction,550,650);//酒店名字，550是高，650是宽，根据需要修改。
	//f.action=paction;
	//this.href=paction;
	//f.target="_blank";
	//f.submit();
...
function findPassword(){
 ...
 GB_showCenter("Enjoyable Stars Hotel",url,450,650);//酒店名字，550是高，650是宽，根据需要修改。
}
...
function onLoadWebNews(){
	top.location="/hcrm/website/hotelhome/"+hotelCode+"/en/allNews.jsp?hotelCode="+hotelCode+"&langCode="+language+"&type=1";
}

//促销更多
function onLoadPromotions(){
	top.location="/hcrm/website/hotelhome/"+hotelCode+"/en/allNews.jsp?hotelCode="+hotelCode+"&langCode="+language+"&type=0";
}
